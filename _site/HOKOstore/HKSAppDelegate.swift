...
class HKSAppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func setupHoko() {
        ...
        Hoko.deeplinking().mapRoute("product/:product_id", toTarget: { deeplink in
            if let coupon = deeplink.metadata?["coupon"], let discount = deeplink.metadata?["value"] {
                let vc = HKSStoreProductViewController()
                let img = UIImage(named: "a")!
                var prod = HKSProduct("dasdadaDSAd", description: "DSADAD", image: UIImage(named: "a")!, price: 200)
                vc.product = prod
                HOKNavigation.presentViewController(vc, animated: true)
            }
        })
    }
    ...
    func showSignInViewIfNeeded() {
        ...
        if !defaults.boolForKey(HKSUserSignedInDefaultsKey) {
